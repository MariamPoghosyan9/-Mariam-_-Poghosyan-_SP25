...
WITH employee_revenue AS (
    ...
    FROM public.staff s
    LEFT JOIN public.rental r ON s.staff_id = r.staff_id  -- Rental comes before payment
    LEFT JOIN public.payment p ON r.rental_id = p.rental_id  -- Payment is joined after rental
    WHERE p.payment_date IS NOT NULL  -- Ensuring we do not filter out NULLs incorrectly
...
SELECT 
    s.store_id, 
    e.first_name || ' ' || e.last_name AS employee_name,  -- Fixed string concatenation
    te.total_revenue
...
WHERE te.store_rank = 1  -- Choose the last store the employee worked at
